...
//
// Não confia em req.query: nenhum outro endpoint deste projeto usa, e o readBody logo
// abaixo existe justamente porque req.body também não veio pronto. Se o runtime não
// popular req.query, o parâmetro é lido de req.url — sem isso, ?e= seria silenciosamente
// ignorado e todo QR cairia na pesquisa resolvida por data.
function eventoDaQuery(req) {
  let bruto = (req.query && req.query.e) || '';
  if (Array.isArray(bruto)) bruto = bruto[0];
  if (!bruto && req.url) {
    const m = /[?&]e=([^&#]*)/.exec(req.url);
    if (m) { try { bruto = decodeURIComponent(m[1]); } catch (e) { bruto = m[1]; } }
  }
  bruto = String(bruto || '').trim().toLowerCase();
